...
// 添加视频记录（追加到 videos 数组）
function addExerciseVideo(exerciseId, video) {
  return db.collection('exercise_videos').where({ exerciseId }).get().then(res => {
...
function deleteWorkoutHistory(id) {
  return db.collection('workout_history').doc(id).remove()
}
...
function loadMoreHistory(lastTimestamp, limit = 20) {
  const _ = db.command
  return db.collection('workout_history')
    .where({ timestamp: _.lt(lastTimestamp) })
    .orderBy('timestamp', 'desc')
    .limit(limit)
    .get()
}
